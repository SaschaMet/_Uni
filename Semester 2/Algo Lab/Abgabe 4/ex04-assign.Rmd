...
svm_radial <- svm(Purchase ~. , data=oj_trn, kernel="radial",  trControl = trctrl,
                    preProcess = c("center", "scale"),
                    tuneGrid = rad_grid )

svm_radial_acc <- postResample(predict(svm_radial, oj_tst), oj_tst$Purchase)
svm_radial_acc["Accuracy"]
```
...
summary_results <- tribble(~model, ~method, ~accuracy,
                           "SVM linear", "svmLinear", as.double(svm_lin_acc["Accuracy"]), 
                           "SVM polynomial", "scmLinear2", as.double(svm_poly_1_acc["Accuracy"]),
                           "SVM radial", "svmRadial", as.double(svm_radial_acc["Accuracy"]), 
                           "Random Forest", "rf", as.double(rf_acc["Accuracy"]))

summary_results
```

Das SVM radial Model hat am besten abgeschnitten, dicht gefolgt von den anderen beiden SVM Modellen. Das radial-Model hat am besten abgeschnitten, da die Variable "Purchase" mit einer solchen Methode besser vorherzusagen ist.
...
Auf den ersten Blick sind die Cluster nicht besonders gut zu unterscheiden Um eine deutlichere Unterscheidung der Cluster zu bekommen, müssten mehrere Parameter mit einbezogen werden.
